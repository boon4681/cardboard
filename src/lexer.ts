...
//             const test = wrapper.test()
//             this.depthDebug.log(chalk.green(wrapper.name), test)
//             const result = wrapper.read()
...
//             }
//             else {
//                 throw new Error(`No viable alternative.\n${chalk.red(this.source.pan(-100, true))} <- missing ${test[1].regex}`)
//             }
...
//         if (this.stack.length == 0) return [false, 'empty lexer']
//         for (const lexer of this.stack) {
//             if (
//                 lexer.test()
//             ) {
